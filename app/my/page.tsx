...
export default function MyPage() {
  ...
  const hydrating = useProfileStore((s) => s.hydrating);
  ...
  return (
    <div className="max-w-[1160px] mx-auto px-5 md:px-10 py-14 space-y-10">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
        {/* 서버에서 내 이력서를 불러오는 중엔 "새 이력서 작성" 칸만 있는 상태가 잠깐 보여서
            원래 있던 이력서가 사라진 것처럼 보일 수 있었다 — 로딩 중엔 카드 자리에 스켈레톤을 둔다. */}
        {hydrating ? (
          Array.from({ length: 2 }).map((_, i) => (
            <div key={i} className="animate-pulse w-full min-h-[360px] rounded-lg bg-neutral-100" aria-hidden="true" />
          ))
        ) : (
          <>
