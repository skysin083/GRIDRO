import { Profile } from "@/types/profile";

const img = (seed: string, n: number) =>
  Array.from({ length: n }, (_, i) => `/dummy-images/${seed}-${i}.svg`);

type RawDummyProfile = Omit<Profile, "bio" | "isNewcomer" | "careers" | "authorTraitsNote" | "contactNote">;

const rawDummyProfiles: RawDummyProfile[] = [
  {
    id: "dummy-1",
    nickname: "연아",
    email: "yeona.art@example.com",
    images: img("gujik-yeona", 3),
    parts: ["채색", "명암"],
    preferredGenres: ["로맨스", "드라마"],
    dislikedGenres: ["액션"],
    tools: [
      { name: "Photoshop", level: "숙련" },
      { name: "Clip Studio Paint", level: "중급" },
    ],
    workStyle: "여성향",
    authorTraits: ["작업 속도 빠름", "소통 원활"],
    workType: "프리랜서",
    contactTime: "평일 오후",
    intro: "잔잔한 감성 로맨스 채색을 가장 잘합니다.",
  },
  {
    id: "dummy-2",
    nickname: "도하",
    email: "doha.line@example.com",
    images: img("gujik-doha", 2),
    parts: ["선화", "콘티"],
    preferredGenres: ["액션", "무협"],
    dislikedGenres: ["로맨스"],
    tools: [{ name: "Clip Studio Paint", level: "숙련" }],
    workStyle: "무관",
    authorTraits: ["피드백 수용 잘함", "꼼꼼함"],
    workType: "고정 어시",
    contactTime: "평일 저녁",
    intro: "역동적인 액션 콘티·선화 전문입니다.",
  },
  {
    id: "dummy-3",
    nickname: "미나",
    email: "mina.bg@example.com",
    images: img("gujik-mina", 4),
    parts: ["배경", "후보정"],
    preferredGenres: ["판타지", "학원물"],
    dislikedGenres: [],
    tools: [
      { name: "Photoshop", level: "숙련" },
      { name: "SAI", level: "중급" },
    ],
    workStyle: "무관",
    authorTraits: ["연락 잘됨", "수정 대응 빠름"],
    workType: "외주(단건)",
    contactTime: "시간 무관",
    intro: "몰입감 있는 배경 작화로 완성도를 높여드립니다.",
  },
  {
    id: "dummy-4",
    nickname: "하늘",
    email: "hanul.color@example.com",
    images: img("gujik-hanul", 3),
    parts: ["밑색", "채색"],
    preferredGenres: ["BL"],
    dislikedGenres: ["무협"],
    tools: [{ name: "Procreate", level: "숙련" }],
    workStyle: "남성향",
    authorTraits: ["작업 속도 빠름"],
    workType: "프리랜서",
    contactTime: "주말",
    intro: "깔끔한 밑색·채색으로 마감을 지킵니다.",
  },
  {
    id: "dummy-5",
    nickname: "지안",
    email: "jian.shade@example.com",
    images: img("gujik-jian", 2),
    parts: ["명암", "채색"],
    preferredGenres: ["스릴러", "드라마"],
    dislikedGenres: ["개그"],
    tools: [
      { name: "Clip Studio Paint", level: "중급" },
      { name: "Photoshop", level: "중급" },
    ],
    workStyle: "무관",
    authorTraits: ["꼼꼼함", "소통 원활"],
    workType: "파트타임",
    contactTime: "평일 오전",
    intro: "어두운 톤의 명암 표현이 강점입니다.",
  },
  {
    id: "dummy-6",
    nickname: "서연",
    email: "seoyeon.full@example.com",
    images: img("gujik-seoyeon", 3),
    parts: ["전공정"],
    preferredGenres: ["로판", "판타지"],
    dislikedGenres: [],
    tools: [
      { name: "Photoshop", level: "숙련" },
      { name: "Medibang", level: "중급" },
    ],
    workStyle: "여성향",
    authorTraits: ["피드백 수용 잘함", "연락 잘됨"],
    workType: "프리랜서",
    contactTime: "평일 오후",
    intro: "콘티부터 후보정까지 전공정 소화 가능합니다.",
  },
  {
    id: "dummy-7",
    nickname: "태양",
    email: "taeyang.line@example.com",
    images: img("gujik-taeyang", 2),
    parts: ["선화", "명암"],
    preferredGenres: ["무협", "액션"],
    dislikedGenres: ["BL"],
    tools: [{ name: "SAI", level: "숙련" }],
    workStyle: "남성향",
    authorTraits: ["작업 속도 빠름", "수정 대응 빠름"],
    workType: "고정 어시",
    contactTime: "평일 저녁",
    intro: "박진감 넘치는 선화와 명암을 담당합니다.",
  },
  {
    id: "dummy-8",
    nickname: "유리",
    email: "yuri.color@example.com",
    images: img("gujik-yuri", 4),
    parts: ["채색", "후보정"],
    preferredGenres: ["학원물", "개그"],
    dislikedGenres: ["스릴러"],
    tools: [
      { name: "Procreate", level: "중급" },
      { name: "Clip Studio Paint", level: "중급" },
    ],
    workStyle: "무관",
    authorTraits: ["소통 원활", "연락 잘됨"],
    workType: "외주(단건)",
    contactTime: "주말",
    intro: "밝고 경쾌한 학원물 채색을 좋아합니다.",
  },
  {
    id: "dummy-9",
    nickname: "아름",
    email: "areum.concept@example.com",
    images: img("gujik-areum", 3),
    parts: ["콘티", "배경"],
    preferredGenres: ["드라마", "로맨스"],
    dislikedGenres: ["무협"],
    tools: [{ name: "Photoshop", level: "중급" }],
    workStyle: "여성향",
    authorTraits: ["꼼꼼함", "피드백 수용 잘함"],
    workType: "프리랜서",
    contactTime: "시간 무관",
    intro: "안정적인 구도의 콘티와 배경을 그립니다.",
  },
];

export const dummyProfiles: Profile[] = rawDummyProfiles.map((p) => ({
  ...p,
  bio: "",
  isNewcomer: false,
  careers: [],
  authorTraitsNote: "",
  contactNote: "",
}));
